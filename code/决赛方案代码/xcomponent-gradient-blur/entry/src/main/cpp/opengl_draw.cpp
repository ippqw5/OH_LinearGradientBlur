...
int32_t OpenglDraw::Init(EGLNativeWindowType window, int width, int height, std::string image)
{
    ...
    LOGI("window size is %{public}d %{public}d", mWidth, mHeight);
    glEnable(GL_DEPTH_TEST);

    mRectangle = new Rectangle();
    mRectangle->Init(mWidth, mHeight, mImage);
    LOGI("Init success.");
...
void OpenglDraw::LinearGradientBlurOption(BlurOption blurOption) {
    if (mRectangle != nullptr) {
        mRectangle->UpdateBlurOption(blurOption);
    }
}

void OpenglDraw::RealTimeSAT(bool is) {
    if (mRectangle != nullptr) {
        mRectangle->RealTimeSAT(is);
    }
}